...
# ML/DataFrame-only version: works on recent Spark/DBR
...
def _binary_curves(scored_df: DataFrame, *, label_col: str, prob_pos_col: str):
    ...
    score_col = (F.col(prob_pos_col)[1] if isinstance(field.dataType, VectorUDT)
                 else F.col(prob_pos_col)).cast("double")

    df = (scored_df
          .select(score_col.alias("score"), F.col(label_col).cast("double").alias("label"))
          .where(F.col("score").isNotNull() & F.col("label").isNotNull()))

    # Need both classes present
    P, N = df.agg(F.sum((F.col("label")==1).cast("int")),
                  F.sum((F.col("label")==0).cast("int"))).first()
    P, N = int(P or 0), int(N or 0)
    if P == 0 or N == 0:
    ...
    ranked = (df
              ...
              .withColumn("cum_tp", F.sum("is_pos").over(w))
              .withColumn("cum_fp", F.sum("is_neg").over(w)))

    # One row per distinct score → max cumulative at that score
    by_t = (ranked.groupBy("score")
            .agg(F.max("cum_tp").alias("TP"), F.max("cum_fp").alias("FP"))
            .orderBy(F.col("score").desc())
            .withColumn("TPR", F.col("TP")/F.lit(P))                # recall
            .withColumn("FPR", F.col("FP")/F.lit(N))
            .withColumn("PREC",
                        F.when((F.col("TP")+F.col("FP"))>0,
                               F.col("TP")/(F.col("TP")+F.col("FP"))).otherwise(F.lit(1.0)))
            .select("FPR","TPR","PREC"))

    # Collect curves (ROC: x=FPR, y=TPR; PR: x=Recall, y=Precision)
    roc_pts = [(0.0, 0.0)] + [(float(r.FPR), float(r.TPR)) for r in by_t.collect()] + [(1.0, 1.0)]
    pr_pts  = [(float(r.TPR), float(r.PREC)) for r in by_t.select("TPR","PREC").collect()]
    if not pr_pts or pr_pts[0][0] > 0.0: pr_pts = [(0.0, 1.0)] + pr_pts
    if pr_pts[-1][0] < 1.0: pr_pts.append((1.0, pr_pts[-1][1]))

    # Trapezoidal AUCs
    def area(points):
        if len(points) < 2: return float("nan")
        pts = sorted(points, key=lambda t: t[0])
        return float(sum((x2-x1)*(y1+y2)/2.0 for (x1,y1),(x2,y2) in zip(pts[:-1], pts[1:])))
