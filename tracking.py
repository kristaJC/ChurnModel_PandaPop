...
from pyspark.ml.tuning import CrossValidatorModel, TrainValidationSplitModel, CrossValidator, ParamGridBuilder
...
def _set_evaluator(estimator, evaluator, collect_submodels=False, metric="areaUnderPR"):
...
def _log_submodels(eval_df, estimator, evaluator, probability_col, positive_index):
    ...
    if isinstance(estimator, (CrossValidatorModel, TrainValidationSplitModel)):
        sub_models = estimator.getCollectSubModels()
    ...
    for sub_model in sub_models:
        ...
        mlflow.log_metrics(metrics, artifact_path=f"./metrics_folder/{sub_model.name}")
        mlflow.log_params({
    ...
    try:
        scored = estimator.transform(eval_df).select(
            F.col(label_col).cast("int").alias(label_col),
            F.col(probability_col).alias(probability_col)
        )
        if persist_eval:
    ...
    mlflow.log_params({
        ...
        }, artifact_path=f"./params_folder/{model.name}")
    ...
    *,
    ...
    with mlflow.start_run(run_name=run_name) as run:
        try:
            ...
            print("Logging Model")
            model = _log_submodels(eval_df, fitted, evaluator, probability_col, positive_index)
